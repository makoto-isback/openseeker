...
export default function SettingsScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* Referral */}
      <ReferralSection walletAddress={walletAddress} serverUrl={serverUrl} />
...
function ReferralSection({ walletAddress, serverUrl }: { walletAddress: string | null; serverUrl: string }) {
  ...
  const loadStats = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${serverUrl}/api/referral/stats?wallet=${walletAddress}`);
      if (res.ok) {
  ...
  const handleCopy = () => {
    if (stats?.link) {
      Clipboard.setString(stats.link);
      setCopied(true);
  ...
  return (
    <>
      <Text style={[styles.sectionTitle, { marginTop: spacing.xxl }]}>Referral</Text>
      <View style={styles.walletCard}>
        {!walletAddress ? (
          <Text style={styles.settingLabel}>Connect wallet to see referral info</Text>
        ) : loading ? (
          ...
          <>
            ...
            <TouchableOpacity onPress={handleCopy}>
              <Text style={{
                ...
                {stats.code} {copied ? '[copied!]' : '[copy]'}
              </Text>
            </TouchableOpacity>
            <Text style={styles.dcaMeta}>Share your link: {stats.link}</Text>

            <View style={[styles.spendRow, { marginTop: spacing.md }]}>
              <View style={styles.spendCard}>
                <Text style={styles.spendLabel}>Referrals</Text>
                <Text style={styles.spendValue}>{stats.referralCount || 0}</Text>
              </View>
              <View style={styles.spendCard}>
                <Text style={styles.spendLabel}>Total Earned</Text>
                <Text style={styles.spendValue}>
                  ${(stats.totalEarnings?.USDC || 0).toFixed(4)}
                </Text>
              </View>
            </View>

            {stats.unpaidEarnings && (stats.unpaidEarnings.USDC > 0 || stats.unpaidEarnings.SOL > 0) && (
              <Text style={[styles.dcaMeta, { marginTop: spacing.sm, color: colors.green }]}>
                Unpaid: {stats.unpaidEarnings.USDC > 0 ? `$${stats.unpaidEarnings.USDC.toFixed(4)} USDC` : ''}
                {stats.unpaidEarnings.SOL > 0 ? ` ${stats.unpaidEarnings.SOL.toFixed(6)} SOL` : ''}
              </Text>
            )}

            <TouchableOpacity
              style={[styles.debugButton, { marginTop: spacing.md }]}
              onPress={loadStats}
            >
          ...
          <>
            <Text style={styles.settingLabel}>10% Revenue Share</Text>
            <Text style={styles.dcaMeta}>
              Earn 10% of all platform fees generated by users you refer. Share your code during onboarding.
            </Text>
            <TouchableOpacity
              style={[styles.debugButton, { marginTop: spacing.md }]}
              onPress={loadStats}
            >
              <Text style={styles.debugButtonText}>[generate code]</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
